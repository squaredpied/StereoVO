...
def normalize_points(points, K):
    """
    Normalize pixel coordinates using the camera intrinsic matrix.
    
    Parameters:
        points (np.ndarray): Array of shape (N, 2) representing pixel coordinates.
    ...
    normalized_points = (np.linalg.inv(K) @ points_homog.T).T  # Apply K^-1
...
for i, (current_left_img, current_right_img) in enumerate(dataset.gray):
    ...
    # Update plot with the current trajectory
    ax.clear()
